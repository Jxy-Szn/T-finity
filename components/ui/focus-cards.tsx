  ...
  ({
    ...
    >
      ...
      >
        {card.href ? (
          <Link href={card.href}>
            <div className="text-lg md:text-xl font-medium bg-clip-text text-transparent bg-gradient-to-b from-neutral-50 to-neutral-200 hover:underline">
              {card.title}
            </div>
          </Link>
        ) : (
...
export function FocusCards({ cards }: { cards: { title: string; src: string; href?: string }[] }) {
  const [hovered, setHovered] = useState<number | null>(null);

  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto md:px-6 w-full">
